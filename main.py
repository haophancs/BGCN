...
def main():
    ...
    if 'pretrain' in CONFIG:
        pretrain = torch.load(CONFIG['pretrain'], map_location=device)
        print('load pretrain')
    else:
        pretrain = None
    #  graph
    ...
    for lr, decay, message_dropout, node_dropout \
        ...
        if CONFIG['model'] == 'BGCN':
            ...
            info = BGCN_Info(128, decay, message_dropout, node_dropout, 2)
            model = BGCN(info, assist_data, graph, device, pretrain=pretrain).to(device)
